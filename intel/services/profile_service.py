...
BUCKET_ORDER = ["Combat ships", "Big miners", "Small miners", "Capsules", "Haulers", "Explorers", "Others"]
BUCKET_ICON = {"Combat ships": "bi-rocket-takeoff", "Big miners": "bi-truck", "Small miners": "bi-gem",
               "Capsules": "bi-person", "Haulers": "bi-box-seam", "Explorers": "bi-binoculars",
               "Others": "bi-three-dots"}
SPACE_ORDER = ["High sec", "Low sec", "Null sec", "Wormhole", "Others"]
SPACE_CLS = {"High sec": "sec-high", "Low sec": "sec-low", "Null sec": "sec-null",
             "Wormhole": "sec-wh", "Others": "sec-other"}
SPACE_ABBR = {"High sec": "HS", "Low sec": "LS", "Null sec": "NS", "Wormhole": "WH", "Others": "Others"}
# raw killmail space -> display category (Pochven/abyssal/anything else folds into Others)
_SPACE_CAT = {"High sec": "High sec", "Low sec": "Low sec", "Null sec": "Null sec", "Wormhole": "Wormhole"}


def _space_cat(raw):
    return _SPACE_CAT.get(raw, "Others")
# condensed target groups (collapsed row) -> set of buckets
CONDENSED = {"combat": {"Combat ships"}, "miners": {"Big miners", "Small miners"},
             "explorers": {"Explorers"}, "other": {"Capsules", "Haulers"}}
FILTER_KEYS = ("region", "const", "system", "ship", "target")
_WINDOW_DAYS = {"30": 30, "90": 90, "180": 180, "365": 365}
_RECENT_MAX = 200
_TOP_SHIPS = 8
...
def _window_kills(kills, window):
...
def _most_active_loc(kills_in_space):
...
def build_profile(entry, window, filters):
    ...
    targets = [{"name": b, "icon": BUCKET_ICON[b], "n": bcount.get(b, 0), "pct": _pct(bcount.get(b, 0), n)}
               for b in BUCKET_ORDER]

    space = []
    for s in SPACE_ORDER:
        in_s = [k for k in fk if _space_cat(k["space"]) == s]
        space.append({"name": s, "abbr": SPACE_ABBR[s], "cls": SPACE_CLS[s], "n": len(in_s),
                      "pct": _pct(len(in_s), n), "loc": _most_active_loc(in_s)})

    sgroup = {k["ship"]: (k["group"], k["stealth"]) for k in fk}
    scount = Counter(k["ship"] for k in fk)
    ships_flown = [{"name": name, "group": sgroup[name][0], "is_stealth": sgroup[name][1],
                    "kills": cnt, "pct": _pct(cnt, n)} for name, cnt in scount.most_common(_TOP_SHIPS)]

    return {"character": entry["character"], "reputation": entry["reputation"], "metrics": metrics,
            "targets": targets, "space": space, "ships_flown": ships_flown, "chart": _build_chart(fk, span),
            "has_data": n > 0}


def _match_loss(m, loss_filters):
