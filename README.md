...
Helion is a Django web app of market tools for [EVE Online](https://www.eveonline.com/). It turns market and character data into practical ISK-making views: station-trading spreads across the major trade hubs, hauling profitability between regions and a profit tracker for a haul already made, ice reprocessing margins beside what that business actually
made, what each wormhole fullerite site and each known-space mykoserocin nebula pays for the fleet you fly and what Jita and Amarr pay for the gas you hold, loyalty-point store payouts, shopping lists you can name and keep, a browser for one item's full order book across the ingested regions, a tracker for your courier and item-exchange contracts, a list of everything your characters and corporation own and where it sits, and a history of your own transactions and profits. A reference page lists every published ship hull with its slot layout, its resist profile, its flight and fitting figures and its trait bonuses, read from the static data on each request, and filters those hulls by the weapon and tank bonuses they carry and by the faction skills they need. Price alerts watch any item against a threshold and show up on a bar across every page. The header carries the ISK of every tracked wallet as
...
`/healthz/` is the health URL. It needs no login, and it answers `ok` with 200 only when Postgres and Redis both answer; otherwise it answers `unavailable` with 503. The production healthcheck calls it.
